    ...
    return (
      <main className="min-h-screen bg-emerald-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-xl font-bold mb-4">Level niet gevonden / Level not found</p>
          <button onClick={() => router.push("/vocabulary")} className="bg-emerald-500 text-white px-4 py-2 rounded-lg">
            Terug / Back
          </button>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-purple-50">
          ...
          <div className="flex items-center justify-between py-6">
            ...
            <div className="text-center">
              <h1 className="text-2xl font-bold text-gray-900">
                {config.icon} {config.label_nl}
              </h1>
              <p className="text-sm text-gray-400">{config.label_en}</p>
            </div>
            <div className="w-24" />
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <input
            type="text"
            placeholder="Zoek een woord... / Search a word..."
            value={search}
          ...
          >
            {showArabic ? "Verberg Arabisch / Hide Arabic" : "Toon Arabisch / Show Arabic"}
          </button>
          {/* Exercise shortcut */}
          <button
            onClick={() => router.push(`/vocabulary/exercise?level=${level}`)}
            className={`px-6 py-3 rounded-xl font-medium text-white bg-gradient-to-r ${config.color} transition hover:opacity-90`}
          >
            🧠 Oefen dit niveau / Practise this level
          </button>
        </div>

        {/* Word count */}
        <p className="text-sm text-gray-500 mb-4">{filtered.length} woorden / words</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((word) => (
            <div
              key={word.id}
              className={`bg-white rounded-2xl shadow-md p-5 border ${config.border} hover:shadow-lg transition-shadow`}
            >
              {showArabic && (
                <div className="text-4xl font-bold text-gray-800 text-right mb-2 font-arabic" dir="rtl">
                  {word.arabic}
                </div>
              )}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-gray-400">
            ...
            <p>Geen woorden gevonden / No words found</p>
          </div>
        )}
      </div>
    </main>
