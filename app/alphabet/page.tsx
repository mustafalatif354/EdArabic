...
export default function AlphabetPage() {
  ...
  const isLessonUnlocked = (lessonId: number) =>
    ProgressManager.isLessonUnlocked(lessonId, userProgress)

  const isComprehensiveTestCompleted = () =>
    ProgressManager.isComprehensiveTestCompleted(userProgress)
      ...
      >
          ...
          <div className="absolute inset-0 bg-gray-900/10 backdrop-blur-sm z-20 flex items-center justify-center rounded-3xl">
            <div className="text-center">
              ...
              <div className="text-xs font-bold text-gray-700">
                {lesson.id === 2 ? "Voltooi les 1 eerst" : "Voltooi de vorige les"}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="relative z-10 text-center">
          ...
          {progress.completed ? (
            ...
            <span className="text-gray-400 text-xs">Nog niet gestart / Not started</span>
          )}
        </div>
      </div>
    )
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-purple-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* Page header */}
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">🔤 Arabisch Alfabet</h1>
          <p className="text-gray-500">Arabic Alphabet — Leer alle 28 letters / Learn all 28 letters</p>
        </div>

        {/* Lessons 1 & 2 */}
        <div className="mb-8">
          <h2 className="text-xl font-bold text-gray-700 mb-4">Basis lessen / Foundation lessons</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div
              ...
              className={`group relative bg-white rounded-3xl p-6 shadow-lg transition-all duration-300 overflow-hidden border-2 border-blue-100 ${
                canTakeTest || comprehensiveCompleted
                  ? "hover:shadow-2xl hover:-translate-y-2 cursor-pointer"
                  : "opacity-50 cursor-not-allowed"
              }`}
            >
              {!lesson2Completed && (
                <div className="absolute inset-0 bg-gray-900/10 backdrop-blur-sm z-20 flex items-center justify-center rounded-3xl">
                  <div className="text-center">
                    <div className="text-3xl mb-1">🔒</div>
                    <div className="text-xs font-bold text-gray-700">Voltooi les 2 eerst</div>
                  </div>
                </div>
              )}
              <div className="relative z-10 text-center">
                ...
                <p className="text-xs text-gray-400 mb-3">Comprehensive Test — alle 28 letters</p>
                {comprehensiveCompleted ? (
                  <span className="text-emerald-600 text-sm font-medium">✓ Voltooid / Completed</span>
                ) : canTakeTest ? (
                  <span className="text-blue-600 text-xs font-medium">Min. 80% nodig / required</span>
                ) : (
                  <span className="text-gray-400 text-xs">Voltooi les 2 eerst</span>
                )}
              </div>
            </div>
          </div>
        ...
        {/* Lessons 3–8 */}
        <div>
          <h2 className="text-xl font-bold text-gray-700 mb-4">Gevorderde lessen / Advanced lessons</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {lessons.slice(2).map(renderLessonCard)}
          </div>
        </div>

        {/* Letter exercises shortcut */}
        <div className="mt-10 bg-white rounded-3xl shadow-lg p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-1">Letter oefeningen / Letter exercises</h3>
            <p className="text-gray-500 text-sm">Oefen individuele letters los van de lessen / Practise individual letters outside of lessons</p>
          </div>
          <button
