...
const lessonsData = {
  1: { title: "Les 1: Arabisch Alfabet - Eerste Helft", letters: allArabicLetters.slice(0, 14) },
  2: { title: "Les 2: Arabisch Alfabet - Tweede Helft", letters: allArabicLetters.slice(14, 28) },
  3: { title: "Les 3: Geavanceerde Letters", letters: [
    { symbol: 'ذ', sound: '/sounds/dhal.mp3', name: 'Dhal', description: 'Klinkt als "th" in "that"' },
    { symbol: 'ر', sound: '/sounds/ra.mp3', name: 'Ra', description: 'Klinkt als "r"' },
    { symbol: 'ز', sound: '/sounds/za.mp3', name: 'Za', description: 'Klinkt als "z"' },
    { symbol: 'س', sound: '/sounds/sin.mp3', name: 'Sin', description: 'Klinkt als "s"' },
  ]},
  4: { title: "Les 4: Nieuwe Vormen", letters: [
    { symbol: 'ش', sound: '/sounds/shin.mp3', name: 'Shin', description: 'Klinkt als "sh" in "ship"' },
    { symbol: 'ص', sound: '/sounds/sad.mp3', name: 'Sad', description: 'Emfatische "s" klank' },
    { symbol: 'ض', sound: '/sounds/dad.mp3', name: 'Dad', description: 'Emfatische "d" klank' },
    { symbol: 'ط', sound: '/sounds/ta.mp3', name: 'Ta', description: 'Emfatische "t" klank' },
  ]},
  5: { title: "Les 5: Uitbreiding", letters: [
    { symbol: 'ظ', sound: '/sounds/dha.mp3', name: 'Dha', description: 'Emfatische "z" klank' },
    { symbol: 'ع', sound: '/sounds/ain.mp3', name: 'Ain', description: 'Diepe keelklank' },
    { symbol: 'غ', sound: '/sounds/ghain.mp3', name: 'Ghain', description: 'Raspende keelklank' },
    { symbol: 'ف', sound: '/sounds/fa.mp3', name: 'Fa', description: 'Klinkt als "f"' },
  ]},
  6: { title: "Les 6: Verder Leren", letters: [
    { symbol: 'ق', sound: '/sounds/qaf.mp3', name: 'Qaf', description: 'Diepe "q" klank' },
    { symbol: 'ك', sound: '/sounds/kaf.mp3', name: 'Kaf', description: 'Klinkt als "k"' },
    { symbol: 'ل', sound: '/sounds/lam.mp3', name: 'Lam', description: 'Klinkt als "l"' },
    { symbol: 'م', sound: '/sounds/mim.mp3', name: 'Mim', description: 'Klinkt als "m"' },
  ]},
  7: { title: "Les 7: Bijna Klaar", letters: [
    { symbol: 'ن', sound: '/sounds/nun.mp3', name: 'Nun', description: 'Klinkt als "n"' },
    { symbol: 'ه', sound: '/sounds/haa.mp3', name: 'Ha', description: 'Klinkt als "h"' },
    { symbol: 'و', sound: '/sounds/waw.mp3', name: 'Waw', description: 'Klinkt als "w" of "u"' },
    { symbol: 'ي', sound: '/sounds/ya.mp3', name: 'Ya', description: 'Klinkt als "y" of "i"' },
  ]},
  8: { title: "Les 8: Complete Alfabet", letters: allArabicLetters },
}
...
export default function TestPage() {
  ...
  const generateExercise = useCallback(() => {
    ...
    if (isComprehensiveTest) {
      ...
      for (let i = 0; i < 8; i++) {
        const correctLetter = lesson.letters[Math.floor(Math.random() * lesson.letters.length)]
        const randomLetters = [...lesson.letters].sort(() => Math.random() - 0.5).slice(0, 4)
        if (!randomLetters.find(l => l.symbol === correctLetter.symbol)) randomLetters[0] = correctLetter
        exercises.push({
          type: Math.random() > 0.5 ? 'listen' : 'see',
          correctLetter,
          question: Math.random() > 0.5 ? 'Klik op de letter die je hoort:' : 'Klik op de juiste letter:',
  ...
  const goBack = () => {
    if (isComprehensiveTest) router.push("/home")
  ...
  const handleExerciseAnswer = (selectedLetter: any) => {
    ...
    if (currentExercise < exercises.length - 1) {
      ...
      if (percentage >= requiredScore) {
        completeLesson(percentage, newMistakes === 0)
      } else {
        alert(`Je score was ${percentage}%. Je moet minimaal ${requiredScore}% scoren. Probeer het opnieuw!`)
  ...
  const completeLesson = async (percentage: number, perfectScore: boolean) => {
    ...
    const xp = LevelingSystem.calculateXPForScore(percentage)
    setXpAwarded(xp)

    // Save progress WITH score and xp_earned to the progress table
    await ProgressManager.saveProgress(progressLessonId, true, percentage, xp)
  }
        ...
        {!lessonCompleted && currentEx && exercises.length > 0 ? (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="text-center mb-8">
              ...
              {currentEx.type === 'see' && (
                <div className="text-8xl font-bold mb-6 text-gray-800">{currentEx.correctLetter.symbol}</div>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
            ...
            <h2 className="text-3xl font-bold mb-6 text-green-600">Gefeliciteerd! / Congratulations!</h2>
            {isComprehensiveTest ? (
              <>
                <p className="text-xl mb-4 font-bold text-blue-600">Je hebt de comprehensieve test voltooid!</p>
                <p className="text-lg mb-4">Je kent nu alle 28 letters! Je score: {finalScore}%</p>
                <p className="text-md mb-4 text-gray-600">Je kunt nu doorgaan naar les 3 en verder!</p>
              </>
            ) : (
              <p className="text-xl mb-4">Je hebt de les voltooid! Je score: {finalScore}%</p>
            )}
            {xpAwarded > 0 && (
              <div className="bg-gradient-to-r from-emerald-50 to-blue-50 rounded-xl p-6 mb-6 border-2 border-emerald-200">
                <div className="text-3xl mb-2">⭐</div>
                <p className="text-lg font-bold text-emerald-600 mb-1">+{xpAwarded} XP verdiend / earned!</p>
              </div>
            )}
